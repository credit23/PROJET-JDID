...
void searchRobotsByStatus(RobotStatus status) {
    ...
    if (!found) printf("\033[33mNo robots found in that status!\033[0m\n");
}
...
void resetRobotStatus(int id) {
    ...
    printf("\n\033[32m✅ Robot %d reset to IDLE\033[0m\n", id);
}

void showWorkHistory(const Robot *robot) {
    // Stub: implement based on how history is stored
    if (!robot) return;
